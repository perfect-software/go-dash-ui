...
const ProductionBom = () => {
  ...
  const [tempProductionBomDetails, setTempProductionBomDetails] = useState(null);
 ...
 //   setValidation((prev) => ({ ...prev, bsName: "valid" }));
  ...
  const fetchProductionBomDetails = async (productionBomId) => {
    try {
      if (productionBomId) {
        ...
        const response = await getDataApiService({ productionBomId: productionBomId }, BASE_URL);
        return response;
      }
    } catch (error) {
      console.error(error);
  ...
  const handleEditClick = async () => {
    ...
    try {
      const response = await fetchProductionBomDetails(tempProductionBomDetails[0].productionBomId);
      if (response) {
              ...
              <div className={styles.buttonContainer}>
                {isEditClicked ? (
                  ...
                  <>
                    ...
                    <button
                      className={styles.submitButton}
                      disabled={productionBomData.groups && !productionBomData.groups.length > 0}
